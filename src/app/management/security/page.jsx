...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      const { data: history } = await supabase.from("login_history").select("id, email, timestamp");
  ...
  const updatePassword = async (newPassword) => {
    ...
    if (error) alert("Şifre güncellenirken hata oluştu");
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-6">
        <h2 className="text-2xl font-bold mb-4">Şifre Güncelleme</h2>
        <input
          type="password"
        ...
        <button
          ...
          onClick={() => updatePassword(document.getElementById('newPassword').value)}
        >
          Şifre Güncelle
        </button>
      </div>
      <div className="mt-6">
        <h2 className="text-2xl font-bold mb-4">Giriş Geçmişi</h2>
